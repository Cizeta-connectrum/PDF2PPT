...
`/dashboard` にアクセスすると、他に開発したWebアプリの一覧をボタン一つで起動(新しいタブで開く)できます。一覧の中身はGoogleスプレッドシートをそのまま参照するので、**アプリの追加・削除はスプレッドシートの行を編集するだけ**です(コードのデプロイや認証設定は不要)。

#### セットアップ(初回のみ)

1. 新しいGoogleスプレッドシートを作成する。
2. 1行目(見出し行)に `name` と `url` と入力する(A1セルに`name`、B1セルに`url`)。
3. 2行目以降に、登録したいアプリを1行ずつ追加する(A列にアプリ名、B列にURL)。
4. メニューの「ファイル」→「共有」→「ウェブに公開」を選ぶ。
5. 公開する範囲は対象のシート、形式は「カンマ区切り形式(.csv)」を選び、「公開」をクリックする。
6. 表示された公開用URL(`https://docs.google.com/spreadsheets/d/e/.../pub?output=csv` のような形式)をコピーする。
7. PDF2PPTを動かす環境に、環境変数 `SHEET_CSV_URL` として手順6のURLを設定する。

```bash
SHEET_CSV_URL="https://docs.google.com/spreadsheets/d/e/xxxxx/pub?output=csv" python app/app.py
```

これ以降は、スプレッドシートに行を追加・削除するだけでダッシュボードの一覧が更新されます(ダッシュボードの「更新」ボタンで最新の内容を再取得できます)。環境変数が未設定の場合、ダッシュボードはエラーを表示します。
