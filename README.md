...
`/dashboard` にアクセスすると、他に開発したWebアプリのURLを登録しておき、ボタン一つで起動(新しいタブで開く)できる一覧ダッシュボードが使えます。登録データはローカルには保存せず、Googleスプレッドシート上のデータをGAS(Google Apps Script)のWebアプリ経由で読み書きします。

#### セットアップ(初回のみ)

1. 新しいGoogleスプレッドシートを作成する。
2. メニューの「拡張機能」→「Apps Script」を開く。
3. デフォルトの `Code.gs` の中身を、このリポジトリの [`gas/Code.gs`](gas/Code.gs) の内容で置き換える。
4. 左側の「プロジェクトの設定」(歯車アイコン)→「スクリプト プロパティ」で `API_TOKEN` という名前のプロパティを追加し、値に任意のランダムな文字列(合言葉)を設定する。
5. 「デプロイ」→「新しいデプロイ」→種類の選択で「ウェブアプリ」を選ぶ。
   - **実行するユーザー**: 自分
   - **アクセスできるユーザー**: 全員
   - デプロイすると発行される `.../exec` で終わるURLをコピーする。
6. PDF2PPTを動かす環境(ローカルやCloud Run/Hugging Face Spacesなど)に、以下の環境変数を設定する。
   - `GAS_WEBAPP_URL`: 手順5でコピーしたウェブアプリのURL
   - `GAS_API_TOKEN`: 手順4で設定した`API_TOKEN`と同じ値
...
これでアプリの登録・削除がすべてそのスプレッドシート(シート名: `Apps`)に反映されます。環境変数が未設定の場合、ダッシュボードはエラーを表示します。
