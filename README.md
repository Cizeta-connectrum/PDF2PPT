...
`/dashboard` にアクセスすると、他に開発したWebアプリのURLを登録しておき、ボタン一つで起動(新しいタブで開く)できる一覧ダッシュボードが使えます。登録データは `APPS_DATA_FILE` 環境変数で指定したパス(未指定時は `app/data/apps.json`)にJSONとして保存されます。
